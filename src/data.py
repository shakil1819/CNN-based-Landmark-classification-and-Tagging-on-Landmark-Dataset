...
def get_data_loaders(
    ...
    data_transforms = {
        "train": transforms.Compose([
            # Starting by resizing the image to 256 pixels
            transforms.Resize(256), 
            # Now randomly flipping the image horizontally with a probability of 0.2
            transforms.RandomHorizontalFlip(p=0.2),
            # Applying now random affine transformations within a range of 10 degrees
            transforms.RandomAffine(10),
            # Randomly crop the image to 224x224 pixels
            transforms.RandomCrop(224),
            # Convert the image to a PyTorch tensor
            transforms.ToTensor(),
            # Now finally, normalizing the tensor with the given mean and std
        ...
        "valid": transforms.Compose([
            ...
            # Convert the image to a PyTorch tensor
            transforms.ToTensor(),
            # Normalize the tensor with the given mean and std
            transforms.Normalize(mean.tolist(), std.tolist())
    ...
    data_loaders["valid"] = torch.utils.data.DataLoader(
        # YOUR CODE HERE
        # Creating now our valid data loader,
        valid_data,
    ...
    test_data = datasets.ImageFolder(
        ...
        # Applying now my test transformations,
    ...
    data_loaders["test"] = torch.utils.data.DataLoader(
        ...
        test_data,
        shuffle=False, # So that we don't shuffle the test data, at all
        batch_size=batch_size,
...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    # Getting an iterator from the train dataloader
    ...
    # Now just getting class names from the train data loader
    dataset = data_loaders["train"].dataset # YOUR CODE HERE:
    # Now getting class names from the train data loader
